  ...
  group "root-api-group" {
    ...
    task "api" {
      ...
      template {
        ...
        ENV=docker
        PORT=3001
        POSTGRES=postgresql://{{key "service/postgres/credentials/user"}}:{{key "service/postgres/credentials/password"}}@POSTGRES_HOST:5432/{{key "service/postgres/credentials/database"}}
