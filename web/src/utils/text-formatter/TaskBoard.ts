...
export class TaskBoardFormat extends TextFormatter {
  ...
  protected updated (): string {
    ...
    if (updatedAttributes === 'title') {
      text += `<span class="action">renamed task board <strong>${sanitize(this.data.context.entity.title)}</strong> to <strong>${sanitize(this.data.context.updatedEntity.title)}</strong></span>`
    } else {
      text += `<span class="action">updated task board <strong>${sanitize(this.data.context.entity.title)}</strong></span>`
    }
