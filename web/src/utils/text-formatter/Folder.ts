...
export class FolderFormat extends TextFormatter {
  ...
  protected updated (): string {
    ...
    if (updatedAttributes === 'title') {
      text += `<span class="action">renamed folder <strong>${sanitize(this.data.context.entity.title)}</strong> to <strong>${sanitize(this.data.context.updatedEntity.title)}</strong></span>`
    } else {
      text += `<span class="action">updated folder <strong>${sanitize(this.data.context.entity.title)}</strong>`
    }
