  ...
  protected updated (): string {
    ...
    if (updatedAttributes === 'dueDate') {
      if (this.data.context.entity.dueDate === null) {
        text += `<span class="action">set the due date for task <strong>${sanitize(this.data.context.updatedEntity.title)}</strong> to <strong>${formatDueDateLocal(this.data.context.updatedEntity.dueDate)}</strong></span>`
      } else if (this.data.context.updatedEntity.dueDate === null) {
        text += `<span class="action">removed due date from task <strong>${sanitize(this.data.context.updatedEntity.title)}</strong></span>`
      } else {
        text += `<span class="action">updated due date for task <strong>${sanitize(this.data.context.updatedEntity.title)}</strong> from <strong>${formatDueDateLocal(this.data.context.entity.dueDate)}</strong> to <strong>${formatDueDateLocal(this.data.context.updatedEntity.dueDate)}</strong></span>`
      }
    } else if (updatedAttributes === 'title') {
      text += `<span class="action">renamed task <strong>${sanitize(this.data.context.entity.title)}</strong> to <strong>${sanitize(this.data.context.updatedEntity.title)}</strong></span>`
    } else if (updatedAttributes === 'description') {
      text += `<span class="action">updated the task description for <strong>${sanitize(this.data.context.entity.title)}</strong></span>`
    } else {
      text += `<span class="action">updated task <strong>${sanitize(this.data.context.entity.title)}</strong></span>`
    }
