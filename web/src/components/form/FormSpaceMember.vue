<template>
  <div>
    ...
    >
      ...
      <template
        #feedback
        v-if="($v.invitation.$invalid && $v.invitation.$dirty) || isFormError"
      >
        <div
    ...
    <div
      ...
      v-if="payload.invites.length > 0 || invitationList.length > 0"
    >
      <h4
      ...
      >
        <div class="flex flex-grow">
          ...
          >
            <avatar
              :size="32"
              :src="list.avatar && list.avatar.versions ? list.avatar.versions.default.location : ''"
              :username="memberName(list)"
            ></avatar>
            <span class="text-gray-900 self-center email">{{ list.email }}</span>
          </div>
          <div
            class="self-center"
            v-if="list.accepted == false"
          >

            <span class="pending">PENDING</span>
            <span
              class="invite-again"
              @click="addInvitationList(list.email, list.role, true)"
            >Invite again</span>
          </div>
        </div>
        ...
        <span
          class="close-icon"
          v-if="currentUser.email !== list.email"
          @click="deleteInvitation(indexList, list.email)"
        >
          <legacy-icon
            name="close"
            size=".9em"
      ...
      >
        ...
        <select
          v-model="payload.invites[index].role"
          class="input input-role"
        >
          <option
            disabled
        ...
        <span
          class="close-icon"
          @click="deleteInvitation(index, invitation)"
        >
          <legacy-icon
            name="close"
            size=".9em"
