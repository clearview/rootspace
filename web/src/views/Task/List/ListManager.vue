<template>
  ...
  <div v-else class="list-manager">
    ...
    <div class="add-another-list">
      <ListAddLaneButton
        @click="addList"
        v-if="childDragEnable && orderedLanes.length !== 0 && !archivedView"
      >
        Add List
      </ListAddLaneButton>
    </div>
...
import { YDoc, TaskId, ClientID, ArchivedViewKey } from '../injectionKeys'
...
export default class ListManager extends Vue {
  ...
  get isKanban (): boolean {
    return (
      this.$route.name === 'TaskPage' || this.$route.name === 'TaskPageWithItem'
    )
  }
  ...
  addList () {
    ...
    this.newList = {
      ...
      position: getNextPosition(
        this.board.taskLists.length,
        this.orderedLanes && this.orderedLanes.length > 0
          ? this.orderedLanes[this.orderedLanes.length - 1].position
          : 0
      ),
  ...
  async reorder (data: any) {
    if (data.moved) {
      const [prevIndex, nextIndex] = getReorderIndex(
        data.moved.oldIndex,
        data.moved.newIndex
      )
      ...
      const newPos = getReorderPosition(
        prev ? prev.position : 0,
        next
          ? next.position
          : getNextPosition(this.board.taskLists.length, prev.position)
      )
